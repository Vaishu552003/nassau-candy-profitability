...
4. Wait for the build to finish. Copy the app URL from the dashboard — it should end with **`.streamlit.app`** (Streamlit shows the full link).
...
| Wrong / old URL               | Open **My apps** → select your app → copy the URL that ends with **.streamlit.app**.                           |
...
This repo includes **`runtime.txt`** with `python-3.11`. In Streamlit Cloud you can also set Python under **Advanced settings** when you deploy.